...
| Utility DB wrappers | `UTIL_SYSTEM.md` | `include/util/db/`, `include/util/common/` | `TEST_MAP.md` -> util db and status | util tests are registered under `unit.core::...`, not `unit.util::...` |
| Utility STFT, OpenCL, waveform, WebP | `UTIL_SYSTEM.md`, `GLOSSARY.md` | `include/util/function/stft/`, `include/util/function/image/`, `include/util/common/BackendLoader/` | `TEST_MAP.md` -> util signal and image | runtime backend fallback behavior lives in util tests, not in the core facade |
...
| Control docs and public docs | `INDEX.md`, `NOW.md`, `INVARIANTS.md`, `VERIFY.md` | `AGENT_DOCS/`, `README.md` | `VERIFY.md` quick doc checks | `docs/` is generated output and `BluePrint_PDJE/` is archive only |
