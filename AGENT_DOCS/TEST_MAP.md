...
| --- | --- | --- | --- | --- |
| Full unit suite | `tests/unit/*` | `ctest --test-dir ./build -L unit --output-on-failure` | `./build/pdje_unit_core`, `./build/pdje_unit_input`, `./build/pdje_unit_judge`, `./build/pdje_unit_util`, `./build/pdje_onnxruntime_smoke` | best default after cross-cutting changes |
...
| Util DB and status | `tests/unit/util/db_*.test.cpp`, `status.test.cpp`, `public_headers.test.cpp` | `ctest --test-dir ./build -R 'unit.util::(rocksdb|annoy|sqlite|util)' --output-on-failure` | `./build/pdje_unit_util` | util doctest cases are registered under the `unit.util::...` prefix |
| Util signal, runtime, image, WebP | `tests/unit/util/opencl_*.test.cpp`, `stft_parallel.test.cpp`, `waveform_webp.test.cpp`, `webp_writer.test.cpp` | `ctest --test-dir ./build -R 'unit.util::(backendless|post process rgb mode|stft|mel filter bank|waveform|encode_waveform_webps|encode_webp|write_webp)' --output-on-failure` | `./build/pdje_unit_util`, `./build/pdje_util_cmrc_probe` | use the util binary when the regex becomes too brittle |
