...
| Source defaults | defaults come from `cmakes/Options.cmake` | local `build/` caches can drift | `INVARIANTS.md`, `VERIFY.md`, `README.md` |
| Shared preset matrix | use root `CMakePresets.json` with repo-root `./build`, `Release` and `RelWithDebInfo`, and `PDJE_DYNAMIC=ON` everywhere | keeps cache location and mode names stable across platforms | `INVARIANTS.md`, `VERIFY.md`, `PROJECT_SKILLS.md`, `README.md` |
...
| Windows preset wrapper | `windows_conf_and_build.bat` activates the Conan/MSVC environment and runs the matching Windows preset configure+build in one `cmd` process, with an explicit `on/off` switch for `--fresh` | avoids shell-environment loss while still letting callers choose whether to rebuild configure state from scratch | `VERIFY.md`, Windows helper scripts, future preset docs |
| Input platform gate | `PDJE_DEVELOP_INPUT=ON` on Linux and Windows, forced `OFF` on macOS | current source tree only supports input and judge on Linux and Windows | `INVARIANTS.md`, `README.md` |
