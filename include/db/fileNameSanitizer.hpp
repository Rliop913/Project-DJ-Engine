 ...
 *
 * This class provides static methods to encode and decode file names using a URL-safe
 * base64 encoding. This is useful for ensuring that file names do not contain
 * characters that are incompatible with the file system.
 */
class PDJE_Name_Sanitizer{
     ...
     * @return An optional containing the sanitized file name, or `std::nullopt` if an error occurs.
     */
    static std::optional<SANITIZED> sanitizeFileName(const std::string& fileName);

    /**
     * @brief De-sanitizes a file name that was sanitized with `sanitizeFileName`.
     *
