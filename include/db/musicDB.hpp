...
struct PDJE_API musdata {
  public:
    ...
    BIN             bpmBinary;
    double          bpm = -1.0;
    /// @brief this means the first beat's position. The measurement unit is a
    /// PCM frame. For higher precision, double value is stored as strings in
    /// Cap’n Proto, so the corresponding variable is also a string.
     ...
     * Leave parameters blank (or set to a negative value for bpm) if filtering
     * by that field is not required.
     *
     * @param title__ The title of the music.
     * @param composer__ The composer of the music.
     * @param musicPath__ Path to the music file, relative to the root of the
     * database.
     * @param bpm__ The BPM (beats per minute) of the music. Use a negative
     * value to ignore this field.
     */
