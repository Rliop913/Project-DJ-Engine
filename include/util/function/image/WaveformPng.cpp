...
#include "WaveformPngHighway-inl.h"
...
struct WaveformJob {
    ...
    std::size_t       png_index = 0;
    EncodedPngBytes  *output_slot = nullptr;
};
...
wrap_job_status(const common::Status &status, const WaveformJob &job)
{
    return { status.code,
             "Waveform job channel " + std::to_string(job.channel_index) +
                 " png " + std::to_string(job.png_index) + ": " +
                 status.message };
}
                                ...
                                std::span<float>       maxs_out)
{
    ...
    if (chunk_pcm.size() != expected_sample_count) {
        return common::Result<void>::failure(
            { common::StatusCode::invalid_argument,
              "Waveform chunk span size does not match pcm_per_pixel * x_pixels_per_png." });
    }

    if (mins_out.size() != x_pixels_per_png || maxs_out.size() != x_pixels_per_png) {
        return common::Result<void>::failure(
            { common::StatusCode::invalid_argument,
              "Waveform extrema buffers must match x_pixels_per_png." });
    }
                     ...
                     function::EvalOptions        options)
{
    ...
    WaveformPngBatch             batch(channels.size());
    ...
    for (std::size_t channel_index = 0; channel_index < channels.size();
        ...
        batch[channel_index].resize(png_count);

        std::size_t next_total_job_count = 0;
        if (!detail::checked_add_waveform(total_job_count, png_count, next_total_job_count)) {
            return common::Result<WaveformPngBatch>::failure(
                { common::StatusCode::invalid_argument,
    ...
    for (std::size_t channel_index = 0; channel_index < channels.size();
        ...
        for (std::size_t png_index = 0; png_index < batch[channel_index].size();
             ++png_index) {
            const std::size_t sample_offset = png_index * chunk_sample_count;
            jobs.push_back({ .samples = channel_pcm.data() + sample_offset,
                             .sample_count = chunk_sample_count,
                             .channel_index = channel_index,
                             .png_index = png_index,
                             .output_slot = &batch[channel_index][png_index] });
        }
    }
    ...
    auto worker_fn = [&]() {
        detail::WorkerScratch scratch {
            .column_mins = std::vector<float>(args.x_pixels_per_png, 0.0f),
            .column_maxs = std::vector<float>(args.x_pixels_per_png, 0.0f),
    ...
    if (!first_error.ok()) {
        return common::Result<WaveformPngBatch>::failure(first_error);
    }
