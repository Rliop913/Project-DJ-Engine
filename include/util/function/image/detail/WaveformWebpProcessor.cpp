...
unsigned int
hop_length(const EncodeWaveformWebpStftArgs &args) noexcept
{
    const auto window = static_cast<unsigned int>(1u << args.window_size_exp);
    return std::max(1u,
                    static_cast<unsigned int>(static_cast<float>(window) *
                                              (1.0f - args.overlap_ratio)));
}

PDJE_PARALLEL::STFTRequest
make_request(const EncodeWaveformWebpStftArgs &args)
{
...
StftColorMapper::StftColorMapper(const Args &args)
    : args_(args.waveform), stft_args_(args.stft),
...
StftColorMapper::Prepare(const WaveformJob &job)
{
    ...
    auto [real, imaginary] =
        stft_.calculate(stft_pcm_, make_request(stft_args_));
    if (!imaginary.empty()) {
        throw support::job_error(job,
                                 "STFT RGB output contained imaginary data");
    }
    if (real.empty()) {
...
StftColorMapper::ColorAt(std::size_t column) const
{
    const auto offset = column * 3;
    return { column_rgb_[offset],
             column_rgb_[offset + 1],
             column_rgb_[offset + 2] };
}

void
StftColorMapper::MapStftRgbToColumns(std::span<const float> frames)
{
    ...
    if (column_rgb_.size() !=
        ...
        throw std::runtime_error(
            "Waveform RGB buffer does not match image width.");
    }
