...
std::size_t
ceil_divide(std::size_t dividend, std::size_t divisor) noexcept
{
...
WaveformPlanBuilder::Build() const
{
    ...
    for (auto &channel : plan.batch)
        channel.resize(images_per_channel);
    plan.jobs.reserve(
        support::checked_multiply(plan.channels.size(), images_per_channel));

    for (std::size_t channel = 0; channel < plan.channels.size(); ++channel) {
        for (std::size_t image = 0; image < images_per_channel; ++image) {
            plan.jobs.push_back({ .samples = plan.channels[channel].data() +
...
WaveformPlanBuilder::Validate() const
{
    ...
    if (args_.channel_count == 0 || args_.y_pixels == 0 ||
        ...
        throw std::invalid_argument(
            "Waveform dimensions must be greater than zero.");
    }
    if (args_.compression_level < -1 || args_.compression_level > 9) {
        throw std::invalid_argument(
            "Waveform compression level must be between -1 and 9.");
    }
    ...
    if (mel.n_fft != static_cast<int>(1u << stft_args_->window_size_exp) ||
        ...
        throw std::invalid_argument(
            "Waveform mel filter bank is invalid for RGB output.");
    }
}
...
{
    return support::checked_multiply(args_.pcm_per_pixel,
...
WaveformPlanBuilder::ComputeBufferSizes() const
{
    ...
    output.image_byte_count =
        support::checked_multiply(output.row_stride, args_.y_pixels);
    output.column_rgb_byte_count =
        support::checked_multiply(args_.x_pixels_per_image, 3);
    return output;
}

std::vector<std::vector<float>>
WaveformPlanBuilder::SplitChannels(std::size_t chunk_sample_count) const
{
    ...
    std::vector<std::vector<float>> channels(args_.channel_count,
