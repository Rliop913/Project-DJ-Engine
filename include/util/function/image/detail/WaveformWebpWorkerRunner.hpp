...
class WaveformWorkerRunner {
  public:
    ...
    Run(WaveformEncodePlan &plan, ProcessorFactory &&create_processor) const
    {
        ...
        const auto worker_count =
            ResolveWorkerCount(requested_workers_, plan.jobs.size());
        ...
        const auto worker = [&] {
            try {
                ...
                while (!stop.load(std::memory_order_acquire)) {
                    ...
                    if (index >= plan.jobs.size())
                        return;
                    const auto &job = plan.jobs[index];
                    if (job.samples == nullptr || job.output_slot == nullptr) {
                        throw support::job_error(
                            job, "job is missing sample or output storage");
                    }
  ...
  private:
    static std::size_t
    ResolveWorkerCount(std::size_t requested, std::size_t jobs) noexcept
    {
        auto count =
            requested == 0
