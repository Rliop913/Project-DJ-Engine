...
std::size_t bytes_per_pixel(RasterPixelFormat format)
{
    switch (format) {
    case RasterPixelFormat::gray8: return 1;
    case RasterPixelFormat::gray_alpha8: return 2;
    case RasterPixelFormat::rgb8: return 3;
    case RasterPixelFormat::rgba8: return 4;
    }
    throw std::invalid_argument("Unsupported raster pixel format.");
}

ImageLayout validate(const RasterImageView &image)
{
    if (image.pixels.data() == nullptr || image.width == 0 || image.height == 0) {
        throw std::invalid_argument("Raster image data and dimensions are required.");
    }
    if (image.width > 16383 || image.height > 16383) {
        throw std::invalid_argument("Raster image dimensions exceed WebP limits.");
    }
    ImageLayout layout {
        .row_bytes = checked_multiply(image.width, bytes_per_pixel(image.pixel_format))
    };
    layout.stride = image.stride == 0 ? layout.row_bytes : image.stride;
    if (layout.stride < layout.row_bytes) {
        throw std::invalid_argument("Raster image stride is smaller than a packed row.");
    }
    const auto tail = checked_multiply(layout.stride, image.height - 1);
    if (tail > std::numeric_limits<std::size_t>::max() - layout.row_bytes) {
        throw std::overflow_error("Raster image buffer size calculation overflowed.");
    }
...
std::vector<std::uint8_t> pack_rgba(const RasterImageView &image,
    ...
    for (std::size_t row = 0; row < image.height; ++row) {
        ...
        for (std::size_t column = 0; column < image.width; ++column) {
            ...
            case RasterPixelFormat::gray8:
                rgba[0] = rgba[1] = rgba[2] = pixel[0]; rgba[3] = 255; break;
            case RasterPixelFormat::gray_alpha8:
                rgba[0] = rgba[1] = rgba[2] = pixel[0]; rgba[3] = pixel[1]; break;
            case RasterPixelFormat::rgb8:
                rgba[0] = pixel[0]; rgba[1] = pixel[1]; rgba[2] = pixel[2]; rgba[3] = 255; break;
            case RasterPixelFormat::rgba8:
                std::memcpy(rgba, pixel, 4); break;
            }
        }
    }
    return output;
...
std::vector<std::uint8_t> encode_webp(const EncodeWebpArgs &args)
{
    if (args.compression_level < -1 || args.compression_level > 9) {
        throw std::invalid_argument("WebP compression level must be between -1 and 9.");
    }
    ...
    if (args.image.pixel_format == RasterPixelFormat::rgba8) {
        if (layout.stride > static_cast<std::size_t>(std::numeric_limits<int>::max())) {
    ...
    const auto size = WebPEncodeLosslessRGBA(pixels,
...
void write_webp(const WriteWebpArgs &args)
{
    ...
    if (!output) throw std::runtime_error("Failed to open WebP output path.");
    output.write(reinterpret_cast<const char *>(bytes.data()),
                 static_cast<std::streamsize>(bytes.size()));
    if (!output.good()) throw std::runtime_error("Failed to write WebP output.");
}
