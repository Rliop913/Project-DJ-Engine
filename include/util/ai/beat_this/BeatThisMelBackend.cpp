...
PdjeMelSpectrogramBackend::PdjeMelSpectrogramBackend(
    ...
    : window(window), mel_formula_(melFormula), mel_norm_(melNorm)
{
}

Spectrogram
PdjeMelSpectrogramBackend::ComputeLinearMel(
    const std::span<const float> samples, const BeatThisFrontendConfig &config)
{
    ...
    return Spectrogram{
        .num_frames = static_cast<int>(
            mel.size() / static_cast<std::size_t>(config.num_mels)),
