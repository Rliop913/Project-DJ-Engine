...
template <typename T> class PDJE_Buffer_Arena {
  private:
    ...
    PDJE_IPC_UTILS::IPCSharedMem<T,PDJE_IPC_UTILS::PDJE_IPC_RW> buf1;
    PDJE_IPC_UTILS::IPCSharedMem<T,PDJE_IPC_UTILS::PDJE_IPC_RW> buf2;
    PDJE_IPC_UTILS::IPCSharedMem<uint64_t, PDJE_IPC_UTILS::PDJE_IPC_RW> first_count;
    PDJE_IPC_UTILS::IPCSharedMem<uint64_t, PDJE_IPC_UTILS::PDJE_IPC_RW> second_count;
    
    PDJE_IPC_UTILS::IPCSharedMem<std::atomic_flag, PDJE_IPC_UTILS::PDJE_IPC_RW> lock;
    PDJE_IPC_UTILS::IPCSharedMem<uint8_t, PDJE_IPC_UTILS::PDJE_IPC_RW> buf_first;
...
public:
    ...
    PDJE_Buffer_Arena(const uint64_t count)
    {
        ...
        buf1.MakeIPCSharedMemory(
            #ifdef WIN32
            std::filesystem::path(L"Local\\PDJE_INPUT_SHMEM_BUF_FIRST" + ID.wstring()),
            #else
        ...
        buf2.MakeIPCSharedMemory(
            #ifdef WIN32
            std::filesystem::path(L"Local\\PDJE_INPUT_SHMEM_BUF_SECOND" + ID.wstring()),
            #else
        ...
        first_count.MakeIPCSharedMemory(
            #ifdef WIN32
            std::filesystem::path(L"Local\\PDJE_INPUT_SHMEM_BUF_COUNT_FIRST" + ID.wstring()),
            #else
            std::filesystem::path("PDJE_INPUT_SHMEM_BUF_COUNT_FIRST" + ID.string()),
            #endif
            1
        );
        second_count.MakeIPCSharedMemory(
            #ifdef WIN32
            std::filesystem::path(L"Local\\PDJE_INPUT_SHMEM_BUF_COUNT_SECOND" + ID.wstring()),
            #else
            std::filesystem::path("PDJE_INPUT_SHMEM_BUF_COUNT_SECOND" + ID.string()),
            #endif
            1
        );
        lock.MakeIPCSharedMemory(
            #ifdef WIN32
            std::filesystem::path(L"Local\\PDJE_INPUT_SHMEM_LOCK" + ID.wstring()),
            #else
        ...
        buf_first.MakeIPCSharedMemory(
            #ifdef WIN32
            std::filesystem::path(L"Local\\PDJE_INPUT_SHMEM_BUFFER_SWITCH" + ID.wstring()),
            #else
            std::filesystem::path("PDJE_INPUT_SHMEM_BUFFER_SWITCH" + ID.string()),
            #endif
            1
        );
