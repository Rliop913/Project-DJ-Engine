...
namespace PDJE_CRYPTO{
    ...
    public:
        Hash();
        ~Hash() = default;
        std::string TextHash(const std::string& txt);
        std::string FileHash(const fs::path& fp);
    };

    class PSK{
        public:
        std::vector<uint8_t> psk;
        bool Gen(const std::string& algo="AES-256/GCM");
        
        PSK();
        ~PSK() = default;
        std::string Encode();
        bool Decode(const std::string& hex);
