...
using PDJE_NAME = std::string;
    class ChildProcess {
        ...
        ChildProcess()
        {
            ...
            server.Get("/lsdev", [&](const httplib::Request &req, httplib::Response &res){
            ...
            server.Post("/config", [&](const httplib::Request & req, httplib::Response &res){
                try{
                    ...
                    res.status = 400;
                    std::string errlog = "INVALID_JSON. why:" + std::string(e.what());
            ...
            server.Post("/shmem", [&](const httplib::Request & req, httplib::Response &res){
                try{
                    ...
                    if(!RecvIPCSharedMem(nj.at("PATH").get<std::string>(),
                        ...
                        throw std::runtime_error("failed to receive ipc shared memory.");
                    }
