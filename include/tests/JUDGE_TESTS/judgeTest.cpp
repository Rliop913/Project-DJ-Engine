...
main()
{
    ...
    for(auto& d : devs){
        if(d.Type == PDJE_Dev_Type::KEYBOARD){
            ...
            conf.MatchType = d.Type;
            conf.MatchDetail = PDJE_KEY::A;
            conf.MatchRail = 1;
    ...
    judge.inits.SetEventRule({.miss_range_microsecond = 100000, .use_range_microsecond = 40000});
    judge.inits.SetInputLine(input.PullOutDataLine());
    PDJE_JUDGE::MISS_CALLBACK missed = [](std::unordered_map<uint64_t, PDJE_JUDGE::NOTE_VEC> misses){
        std::cout << "missed!!!" << std::endl;
    };
    PDJE_JUDGE::USE_CALLBACK used = [](uint64_t railid, bool Pressed, bool IsLate, uint64_t diff){
        std::cout << "used!!!" << std::endl;
    };
    PDJE_JUDGE::MOUSE_AXIS_PARSE_CALLBACK mouse_parse = [](uint64_t             microSecond,
