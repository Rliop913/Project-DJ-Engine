...
Compilation options: -lang cpp -light -it -nvi -ct 1 -mapp -cn FlangerFAUSTMan -scn FlangerMan -es 1 -exp10 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 64
------------------------------------------------------------ */
...
class FlangerFAUSTMan final : public FlangerMan {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -light -it -nvi -ct 1 -mapp -cn FlangerFAUSTMan -scn FlangerMan -es 1 -exp10 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 64");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (count - 64); vindex = vindex + 64) {
			...
			/* Compute code */
			for (int i = 0; i < vsize; i = i + 1) {
				fYec1[(i + fYec1_idx) & 127] = 0.8f * fRec3[i - 1] - float(input1[i]);
				fRec3[i] = fYec1[(i + fYec1_idx - std::min<int>(21, std::max<int>(0, iZec10[i]))) & 127] * (fZec12[i] + (1.0f - fZec9[i])) + (fZec9[i] - fZec12[i]) * fYec1[(i + fYec1_idx - std::min<int>(21, std::max<int>(0, iZec10[i] + 1))) & 127];
			}
			/* Post code */
