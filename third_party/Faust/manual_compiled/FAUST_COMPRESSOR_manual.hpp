...
Compilation options: -lang cpp -light -it -nvi -ct 1 -mapp -cn CompressorFAUSTMan -scn CompressorMan -es 1 -exp10 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 64
------------------------------------------------------------ */

#ifndef  __CompressorFAUSTMan_H__
#define  __CompressorFAUSTMan_H__

#ifndef FAUSTFLOAT
#define FAUSTFLOAT float
#endif 

#include "compressorMan.hpp"
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <math.h>
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS CompressorFAUSTMan
#endif
...
static float CompressorFAUSTMan_faustpower2_f(float value) {
	return value * value;
}

class CompressorFAUSTMan final : public CompressorMan {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -light -it -nvi -ct 1 -mapp -cn CompressorFAUSTMan -scn CompressorMan -es 1 -exp10 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0 -vec -lv 0 -vs 64");
	...
	CompressorFAUSTMan* clone() {
		return new CompressorFAUSTMan();
	}
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (vindex = 0; vindex <= (count - 64); vindex = vindex + 64) {
			...
			#pragma clang loop vectorize(enable) interleave(enable)
			for (int i = 0; i < vsize; i = i + 1) {
				output0[i] = FAUSTFLOAT(float(input0[i]) * std::pow(1e+01f, -(fSlow12 * std::max<float>(0.0f, ((iZec3[i] == 0) ? 0.0f : ((iZec3[i] == 1) ? fSlow11 * CompressorFAUSTMan_faustpower2_f(fSlow7 + fZec4[i]) : fZec4[i]))))));
			}
			/* Vectorizable loop 11 */
			/* Compute code */
			#pragma clang loop vectorize(enable) interleave(enable)
			for (int i = 0; i < vsize; i = i + 1) {
				output1[i] = FAUSTFLOAT(float(input1[i]) * std::pow(1e+01f, -(fSlow12 * std::max<float>(0.0f, ((iZec8[i] == 0) ? 0.0f : ((iZec8[i] == 1) ? fSlow11 * CompressorFAUSTMan_faustpower2_f(fSlow7 + fZec9[i]) : fZec9[i]))))));
			}
		}
		/* Remaining frames */
		if (vindex < count) {
