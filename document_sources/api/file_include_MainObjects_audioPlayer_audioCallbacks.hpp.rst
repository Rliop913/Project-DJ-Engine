
.. _file_include_MainObjects_audioPlayer_audioCallbacks.hpp:

File audioCallbacks.hpp
=======================

|exhale_lsh| :ref:`Parent directory <dir_include_MainObjects_audioPlayer>` (``include\MainObjects\audioPlayer``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS


.. contents:: Contents
   :local:
   :backlinks: none

Definition (``include\MainObjects\audioPlayer\audioCallbacks.hpp``)
-------------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_include_MainObjects_audioPlayer_audioCallbacks.hpp.rst





Includes
--------


- ``MusicControlPannel.hpp`` (:ref:`file_include_audioRender_ManualMix_MusicControlPannel.hpp`)

- ``PDJE_EXPORT_SETTER.hpp`` (:ref:`file_include_PDJE_EXPORT_SETTER.hpp`)

- ``miniaudio.h``

- ``optional``



Included By
-----------


- :ref:`file_include_MainObjects_audioPlayer_audioCallbacks.cpp`

- :ref:`file_include_MainObjects_audioPlayer_audioPlayer.hpp`




Classes
-------


- :ref:`exhale_struct_structaudioEngineDataStruct`

