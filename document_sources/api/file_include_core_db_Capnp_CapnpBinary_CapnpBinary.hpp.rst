
.. _file_include_core_db_Capnp_CapnpBinary_CapnpBinary.hpp:

File CapnpBinary.hpp
====================

|exhale_lsh| :ref:`Parent directory <dir_include_core_db_Capnp_CapnpBinary>` (``include/core/db/Capnp/CapnpBinary``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS


.. contents:: Contents
   :local:
   :backlinks: none

Definition (``include/core/db/Capnp/CapnpBinary/CapnpBinary.hpp``)
------------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_include_core_db_Capnp_CapnpBinary_CapnpBinary.hpp.rst





Includes
--------


- ``MixBinary.capnp.h``

- ``MusicBinary.capnp.h``

- ``NoteBinary.capnp.h``

- ``PDJE_EXPORT_SETTER.hpp`` (:ref:`file_include_global_PDJE_EXPORT_SETTER.hpp`)

- ``PDJE_LOG_SETTER.hpp`` (:ref:`file_include_global_PDJE_LOG_SETTER.hpp`)

- ``capnp/message.h``

- ``capnp/serialize.h``

- ``optional``

- ``vector``



Included By
-----------


- :ref:`file_include_core_db_Capnp_Translators_FrameCalc.hpp`

- :ref:`file_include_core_db_Capnp_Translators_MixTranslator_MixTranslator.hpp`

- :ref:`file_include_core_db_Capnp_Translators_MusicTranslator_MusicTranslator.hpp`

- :ref:`file_include_core_db_Capnp_Translators_NoteTranslator_NoteTranslator.hpp`

- :ref:`file_include_core_editor_pdjeLinter_trackLinter.cpp`

- :ref:`file_include_tests_audioTest.cpp`




Classes
-------


- :ref:`exhale_class_classCapReader`

- :ref:`exhale_class_classCapWriter`

