
.. _file_include_core_editor_editor.hpp:

File editor.hpp
===============

|exhale_lsh| :ref:`Parent directory <dir_include_core_editor>` (``include/core/editor``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS



High level editor interface for :ref:`exhale_class_classPDJE` projects. 



.. contents:: Contents
   :local:
   :backlinks: none

Definition (``include/core/editor/editor.hpp``)
-----------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_include_core_editor_editor.hpp.rst





Includes
--------


- ``EditorArgs.hpp`` (:ref:`file_include_core_editor_TimeLine_JSONWrap_EditorArgs.hpp`)

- ``MusicControlPanel.hpp`` (:ref:`file_include_core_audioRender_ManualMix_MusicControlPanel.hpp`)

- ``MusicJsonHelper.hpp`` (:ref:`file_include_core_editor_TimeLine_JSONWrap_MusicJsonHelper.hpp`)

- ``PDJE_EXPORT_SETTER.hpp`` (:ref:`file_include_global_PDJE_EXPORT_SETTER.hpp`)

- ``PDJE_LOG_SETTER.hpp`` (:ref:`file_include_global_PDJE_LOG_SETTER.hpp`)

- ``TimeLine.hpp`` (:ref:`file_include_core_editor_TimeLine_TimeLine.hpp`)

- ``dbRoot.hpp`` (:ref:`file_include_core_db_dbRoot.hpp`)

- ``exception``

- ``fileNameSanitizer.hpp`` (:ref:`file_include_global_DataLines_fileNameSanitizer.hpp`)

- ``filesystem``

- ``memory`` (:ref:`file_include_input_IPC_memory_ipc_shared_memory.hpp`)

- ``system_error``



Included By
-----------


- :ref:`file_include_core_MainObjects_editorObject_editorObject.cpp`

- :ref:`file_include_core_MainObjects_editorObject_editorObject.hpp`

- :ref:`file_include_core_editor_editor.cpp`




Classes
-------


- :ref:`exhale_struct_structPDJE__Editor_1_1MusicHandleStruct`

- :ref:`exhale_class_classPDJE__Editor`


Functions
---------


- :ref:`exhale_function_editor_8hpp_1ac3f552b1307bd14d1fa2758d78011f94`

