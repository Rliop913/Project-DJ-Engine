...
{
    DecodedImage output;
    REQUIRE(WebPGetInfo(bytes.data(), bytes.size(), &output.width, &output.height) != 0);
...
std::uint8_t alpha_at(const DecodedImage &image, std::size_t x, std::size_t y)
{
    return image.pixels[(y * static_cast<std::size_t>(image.width) + x) * 4 + 3];
}

std::vector<float> color_pcm(std::size_t count)
{
...
PDJE_UTIL::function::image::EncodeWaveformWebpArgs args_for(
    std::span<const float> pcm, std::size_t workers)
{
    return {
        .pcm = pcm,
        .channel_count = 1,
...
TEST_CASE("waveform encoder splits channels and preserves transparent background")
{
...
TEST_CASE("waveform output is deterministic across worker counts")
{
    ...
    const auto parallel = PDJE_UTIL::function::image::encode_waveform_webps(
        args_for(pcm, 4));
    const auto automatic = PDJE_UTIL::function::image::encode_waveform_webps(
        args_for(pcm, 0));
