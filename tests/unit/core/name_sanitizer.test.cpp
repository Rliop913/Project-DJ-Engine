...
TEST_CASE("core: name sanitizer roundtrip preserves empty and punctuation strings")
{
...
TEST_CASE("core: name sanitizer enforces encoded length boundary")
{
    ...
    CHECK(cbase::encoded_size(accepted_len) < 255);
    CHECK(cbase::encoded_size(rejected_len) >= 255);

    CHECK(PDJE_Name_Sanitizer::sanitizeFileName(std::string(accepted_len, 'a'))
              .has_value());
    CHECK_FALSE(PDJE_Name_Sanitizer::sanitizeFileName(
                    std::string(rejected_len, 'a'))
                    .has_value());
}
